...
class Spot():
    def __init__(self,name,jingdu,weidu,type_spot,score=0,clus_flag=0):  # @DontTrace
        '''
        景点对象
        :param name: 景点名称
        :param jingdu:经度
...
def sigle_km(spots_list):
    ...
    #合并 返回 得到分类的list [0,0,0,3,1,1,2,4]
    cluster_list=list_coefficient[1].labels_.tolist()
    cluster_center=list_coefficient[1].cluster_centers_.tolist()
    #标签添加完成
